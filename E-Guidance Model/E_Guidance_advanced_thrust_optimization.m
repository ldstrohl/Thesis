...
% throttle estimate optimization
% Lloyd Strohl
% 5/21/17
...
q = 1; % fixed thrust iterator

throttle_list = [linspace(0.555,0.565,10)]

for thrust_estimate = throttle_list
    t = 0;
k = 1; 
%% Simulation loop
while altitude(k) > 0
   ...
   while p > tgo_tol
       ...
       tgo(k+1) = (m0/(thrust_estimate*T_max/v_ex))*(1-exp(-dV/v_ex)); 
...
figure
plot(throttle_list,fuel_consumption)