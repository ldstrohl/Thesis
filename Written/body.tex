...
Control refers to the manipulation of forces to induce the desired trajectory, orientation, or change of state. For the soft landing problem, these manipulations may be realized through aerodynamic surfaces, thrusters, rockets, or other systems which exert a force on the vehicle.
...
Time derivatives are represented by dot notation as in Equation\:(\ref{eqn:timedotdef}) showing the derivative relationship between position and velocity vectors.
...
\begin{figure}[H]
	...
	\begin{minipage}{4.3 in}
		...
		\caption{CobraMRV Design. Source: CERIMELE, CHRISTOPHER, ROBERTSON, EDWARD, SOSTARIC, RONALD,
			CAMPBELL, CHARLES, ROBINSON, PHIL, MATZ, DANIEL, JOHNSON, BREANNA,
			AND ET AL. ``A Rigid Mid Lift-To-Drag Ratio Approach To Human Mars Entry,
			Descent, And Landing''. \emph{AIAA Paper 2017-1898} (2017) \label{fig:CobraMRV} }
	\end{minipage}
\end{figure}
...
The CobraMRV lift and drag coefficients $C_L$ and $C_D$ are modeled based on vehicle Mach number and angle of attack. Vehicle angle of attack is determined directly from the thrust acceleration vector $\bm{a}_T$ and the vehicle velocity vector $\bm{V}$.
...
\end{table}      
%Equation\:(\ref{eqn:thrustdisp}) shows this calculation for the maximum thrust dispersion, where $T_{max_{nom}}$ is the nominal maximum thrust expected by the guidance computer and $rand$ is a uniformly distributed random number between zero and one.
%
%
%\begin{equation}
...
%do the following:
%\\
%\texttt{ \hspace*{0.5in} $\backslash$begin\{figure\}[H]
%  \\
%  \hspace*{0.75in}
...
%\\
%to produce the figure. The \textbf{[H]} argument to the figure
...
%%        you need to citep a source (of e.g.~a figure) in the caption,
...
\begin{tabularx}{\textwidth}{@{}lYYYYYYYY@{}}
		...
		\toprule                                                                   
		\bfseries Case 
		& \multicolumn{1}{r}{\bfseries 1}
		& \multicolumn{1}{r}{\bfseries 2}
		& \multicolumn{1}{r}{\bfseries 3}
		& \multicolumn{1}{r}{\bfseries 4}
		& \multicolumn{1}{r}{\bfseries 5}
		& \multicolumn{1}{r}{\bfseries 6}
		& \multicolumn{1}{r}{\bfseries 7}
		 \\                                                  
		\toprule                                                                           
...
Introducing dispersion produces Table\:\ref{tab:disppowatmo}. Again we see Case 1 showing failed landings. In atmosphere only 13 landings of 1000 failed, with range misses in excess of 200 m as shown in Figure\:\ref{fig:bulldisppowatmo} and speeds in excess of 32 m/s. These values are lower than in vacuum, though still represent an unacceptable level of risk for a manned mission.
...
Figure\:\ref{fig:hfueldisppowatmo} again demonstrates the characteristic Gaussian distribution in propellant consumption, with fewer outlying low consumption runs in Case 1. Adaptive PDI (Case 7) shows a lower mean than all other cases and maintains a similar distribution as the other cases. Range, speed, and failure histograms are given in the Appendix as they are qualitatively similar to the vacuum performance figures in Section\:\ref{sec:vacperf}.
