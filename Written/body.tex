...
The literature has also given very little attention to the topic of ignition timing, or the initiation of powered descent guidance (PDI). Mission planning involves phases and guidance laws like E-Guidance 


\section{Simulation}

The powered descent simulation (PD Sim) is developed using standard programming techniques and numerical methods. The code is modular to facilitate design and reflect the functions of a mission computer. The simulation has three degrees of freedom (3-DoF) with derived orientation for force modeling.
...
The atmosphere model comes from NASA data using a simplified model blah blah blah

\subsubsection{Lift and Drag}
The vehicle model comes from Cerimele\:et\:al\:\cite{CERIMELE}

\subsubsection{Orientation}
The vehicle's orientation is primarily determined from the thrust command direction. 
